...
//     const grid = new Isotope("section", {//배치할 요소를 가지고 있는 부모 요소명
...
//         //grid 에 저장된 결과 값을 불러와 재정렬 기능 연결
//         grid.arrange({
//             //옵션값으로 sort 변숫값 지정
//             filter : sort
//         });
...
const btns = document.querySelectorAll("main ul li");
let grid; //플러그인의 정봇값이 담긴 변수를 이곳에 전역으로 설정
...
//화면 초기화 함수 정의
function init(){
    //변수 grid 에 담긴 결괏값이 다른 함수인 filter에서도 활용되어야 하므로 전역변수로 선언
...
//정렬 버튼 기능의 함수 정의
function filter(arr){
    //매개 변수 arr를 통해 반복하는 버튼 그룹을 인수로 전달
    for(let el of arr){
